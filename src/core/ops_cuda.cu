...
extern "C" int cuda_matmult(const float *a, const float *b, float *out, int rows, int inner, int cols){
     ...
     */
    status = cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, cols, rows, inner, &alpha, b, cols, a, inner, &beta, out, cols);

    if(status == CUBLAS_STATUS_SUCCESS){
        cudaError_t err = cudaDeviceSynchronize();
        if(err != cudaSuccess) status = CUBLAS_STATUS_EXECUTION_FAILED;
    }

    cublasStatus_t destroy_status = cublasDestroy(handle);
    if(status != CUBLAS_STATUS_SUCCESS || destroy_status != CUBLAS_STATUS_SUCCESS){
        return 1;
    }
    return 0;
}
